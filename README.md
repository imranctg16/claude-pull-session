...
- **Names** come from the app's own per-session metadata at `<config-dir>/sessions/<pid>.json` (`sessionId`, `name`, `status`) — the same name shown at the top of that CLI. Sessions with no such entry fall back to a name derived from their first real message.
