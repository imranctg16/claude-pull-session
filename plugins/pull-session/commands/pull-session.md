...
- **A numbered list of sessions** → present them to me as a **keyboard-selectable menu** (use your interactive option picker), **live sessions first, then most-recent**. **Label each option with a short, human session name — a few words describing what that session is about, drawn from its first message / topic — NOT the id or the number.** Put `instance · project`, the directory, and a ● live tag in the description. The picker shows up to ~4 options, so choose the most relevant and rely on the auto **Other** entry for me to type an id for anything not shown. Keep each session's id internally (don't display it) to run the pull once I choose: `bash "${CLAUDE_PLUGIN_ROOT}/scripts/pull-session.sh" <that session's id>`, then handle the output as below. A **live** pick needs `--force`; a clean merge is better after `/compact` in that session's own terminal.
