...
- **A numbered list of sessions** → present them to me as a **keyboard-selectable menu** (use your interactive option picker), **live sessions first**. **Label each option with the session's name** — the script already resolves each one to its real name (the same label shown at the top of that CLI, e.g. `blue-theme-logo-rebrand`); use that verbatim, NOT the id or the number. Put `instance · project` and the live tag (`● busy` / `○ open` / `idle`) in the description. The picker shows up to ~4 options, so choose the most relevant (prefer live ones) and rely on the auto **Other** entry for me to type an id/number for anything not shown. Keep each session's id internally (don't display it) to run the pull once I choose: `bash "${CLAUDE_PLUGIN_ROOT}/scripts/pull-session.sh" <that session's id>`, then handle the output as below. A **live** pick needs `--force`; a clean merge is better after `/compact` in that session's own terminal.
